...
with Session(engine) as session:
    hick = User(
        name="Hick",
        fullname="sanchos",
        address=[Address(email_address="sanchos@gmail.com")]
    )

    vivian = User(
        name="Vivian",
        fullname="Vivian brasilson",
        address=[Address(email_address="brasilson@hotmail.com")]
    )

    santos = User(
        name="Santos",
        fullname="Santos Silva"
    )

    #enviar para db
    session.add_all([hick, vivian, santos])

    session.commit()


stmt = select(User).where(User.name.in_(["hick", 'vivian']))
